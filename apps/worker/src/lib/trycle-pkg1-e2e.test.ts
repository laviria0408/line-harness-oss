...
describe('Pkg1 総当たり (全選択肢が反応する・wiring 監査の本丸)', () => {
  ...
  /**
   * 確定不能の導線 (v1.6): お悩み自由文入力 か (overhaul ゲートは) メニュー提示。
   * 旧仕様の「スタッフ」即送りは廃止され、まず お悩み/メニューに反応すれば OK。
   */
  function expectsOsayamiOrOverhaul(s: string, region: { kind?: string }): void {
    if (region.kind === 'overhaul') {
      // menus が mock に無い場合は お悩みへフォールバックするので、どちらでも可。
      expect(
        s.includes('action=pkg1_overhaul') || s.includes('どのようなことでお困りですか'),
      ).toBe(true);
    } else {
  ...
  for (const region of REGIONS) {
    it(`region「${region.label}」(${region.value}) の全作業・全種類が反応する`, async () => {
      ...
      // region 選択 → symptoms=null (自由記述 / overhaul ゲート) は確定不能導線へ。
      {
        buildStatefulSupabase();
        const captured: unknown[][] = [];
        const ctx = fakeContext(captured);
        ...
        if (symptoms === null) {
          expectsOsayamiOrOverhaul(s, region);
          return;
        }
      ...
      for (let si = 0; si < symptoms.length; si++) {
        ...
        const isStaffOnly = !hasVariants && !symptom.sample; // sample=null & variant 無し → 確定不能 → お悩み

        if (isStaffOnly) {
          // v1.6: その他 (sample=null) は お悩み自由文マッチングへ (旧スタッフ即送りを置換)。
          expect(s, `${region.label}/${symptom.label}`).toContain('どのようなことでお困りですか');
          continue;
        }
        if (hasVariants) {
          ...
          for (let vi2 = 0; vi2 < symptom.variants!.length; vi2++) {
            ...
            if (!variant.sample) {
              // v1.6: sample=null variant → お悩み自由文マッチングへ。
              expect(s2, tag).toContain('どのようなことでお困りですか');
            } else if (symptom.qty) {
